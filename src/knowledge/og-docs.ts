...
main().catch(e => { console.error(e); process.exitCode = 1; });`,
  ...
  importantNotes: [
    ...
    "Storage indexer URL MUST be HTTPS when used in browser — http:// is blocked by Mixed Content policy",
    "INFT mint flow: upload metadata → verify rootHash is non-empty → THEN call mintINFT. Never mint with an empty rootHash.",
    "estimateGas CALL_EXCEPTION before tx submission = pre-condition failed (bad rootHash, wrong allowance, state conflict)",
    "SES Removing unpermitted intrinsics = MetaMask security sandbox — informational only, not a bug",
